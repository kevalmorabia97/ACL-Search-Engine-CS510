...
more_stopwords = [word.strip() for word in open('data/lemur-stopwords.txt', 'r').readlines()]
...
def get_corpus(filename):
    ...
    for i in range(df.shape[0]):
        link = ''
        if df.iloc[i]['link'] != '':
            link = 'https://www.aclweb.org/anthology/'+df.iloc[i]['link'].strip('.tei.xml')
...
def remove_punctuations(text):
    # remove some punctuations from paper abstract
